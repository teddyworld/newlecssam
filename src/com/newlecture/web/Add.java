...
public class Add extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
	      ...
	      {
	    	  String _bin = request.getParameter("btn");
	    	  if(_bin.equals("application"))
	    	  {
	    		  ServletContext application = request.getServletContext();
	    		  application.setAttribute("a", result);
	    	      out.printf("<p> application 에 저장 완료 </p>");
	    	  }
	    		
	    	  else if(_bin.equals("session"))
	    	  {
	    		  HttpSession session = request.getSession();
	    		  session.setAttribute("s",result);
	    		  out.printf("<p> session 에 저장 완료 </p>");
	    	  }
	    	  
	    	  else if(_bin.equals("cookie"))
	    	  {
	    		  Cookie cookie = new Cookie("C", String.valueOf(result));
	    		  cookie.setMaxAge(24*60*60); //24시간
	    		  response.addCookie(cookie);
	    		  
	    		  out.printf("<p> cookie에 저장 완료 </p>");
	    	  }
