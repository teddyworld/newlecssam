...
<c:set var="logo" value="../resource/images/logo.png"></c:set>
...
<div id="header">
			...
			<div class="top-wrapper">
				...
				<ul id="loginmenu" class="block_hlist">
					...
					<li>
<%-- 			   <c:if test="${empty pageContext.request.userPrincipal }">    JSTL방식의 로그인&로그아웃
						  <a href="../joinus/login">로그인</a>
					   </c:if>
					   <c:if test="${not empty pageContext.request.userPrincipal }">
						  <a href="${ctxName}/j_spring_security_logout">${pageContext.request.userPrincipal.name } 로그아웃</a>
					   </c:if> --%>
					   <a href="../joinus/login">로그인</a>   <%-- Spring방식의 로그인&로그아웃 --%>
					   <a href="${ctxName}/j_spring_security_logout"><security:authentication property="name" /> 로그아웃</a>
					   <security:authentication property="authorities" var="authoritie" />
					   <c:forEach items="${authoritie }" var="auth">
					      ${auth.authority }
					   </c:forEach>
					</li>
					<li>
